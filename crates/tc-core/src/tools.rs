...
struct PluginReference { directory: PathBuf, original_name: String, capabilities: Vec<String> }
...
impl ToolExecutor {
    pub fn new(workspace: impl AsRef<Path>, policy: PermissionPolicy, approval: Arc<dyn Approval>) -> Result<Self> {
        let workspace = workspace.as_ref().canonicalize().with_context(|| format!("workspace does not exist: {}", workspace.as_ref().display()))?;
        let root = env::var("TC_HOME").map(PathBuf::from).unwrap_or_else(|_| dirs::data_local_dir().unwrap_or_else(|| PathBuf::from(".")).join("thinking-computer")).join("plugins");
        let mut plugin_tools = BTreeMap::new();
        for (directory, manifest) in discover_plugins(root)? {
            for tool in manifest.tools {
                let safe = format!("plugin__{}", tool.name.chars().map(|ch| if ch.is_ascii_alphanumeric() || ch == '_' || ch == '-' { ch } else { '_' }).collect::<String>());
                plugin_tools.insert(safe, PluginReference { directory: directory.clone(), original_name: tool.name, capabilities: tool.capabilities });
            }
        }
        Ok(Self { workspace, policy, approval, plugin_tools })
    }
    ...
    fn authorize(&self, capability: Capability, summary: String) -> Result<()> {
        if self.policy.needs_confirmation(&capability) && !self.approval.approve(&ApprovalRequest { capability, summary })? { anyhow::bail!("tool action was denied by the user"); }
        Ok(())
    }

    fn safe_path(&self, requested: &str) -> Result<PathBuf> {
        let candidate = if Path::new(requested).is_absolute() { PathBuf::from(requested) } else { self.workspace.join(requested) };
        let existing = if candidate.exists() { candidate.canonicalize()? } else {
        ...
        if !existing.starts_with(&self.workspace) { anyhow::bail!("path escapes the approved workspace"); }
        Ok(existing)
    }
    ...
    fn write_file(&self, path: &str, content: &str) -> Result<String> {
        let path = self.safe_path(path)?;
        self.authorize(Capability::WriteFile, format!("Write {} bytes to {}", content.len(), path.display()))?;
    ...
    fn shell(&self, command: &str) -> Result<String> {
        ...
        if blocked.iter().any(|needle| command.contains(needle)) { anyhow::bail!("refusing a command that appears privileged or destructive"); }
        self.authorize(Capability::Shell, format!("Run in {}: {}", self.workspace.display(), command))?;
        let output = if cfg!(windows) { Command::new("cmd").args(["/C", command]).current_dir(&self.workspace).output()? } else { Command::new("sh").args(["-lc", command]).current_dir(&self.workspace).output()? };
    ...
    async fn web_search(&self, query: &str) -> Result<String> {
        self.authorize(Capability::WebSearch, format!("Search public web summaries for: {query}"))?;
        let mut url = Url::parse("https://api.duckduckgo.com/")?;
        url.query_pairs_mut().append_pair("q", query).append_pair("format", "json").append_pair("no_html", "1");
        let response: Value = Client::new().get(url).send().await?.error_for_status()?.json().await?;
        let heading = response.get("Heading").and_then(Value::as_str).unwrap_or("No instant answer");
        let abstract_text = response.get("AbstractText").and_then(Value::as_str).unwrap_or("No instant-answer summary was returned. Try a more specific query.");
        let source = response.get("AbstractURL").and_then(Value::as_str).unwrap_or("");
    ...
    fn plugin(&self, safe_name: &str, args: &Value) -> Result<String> {
        ...
        self.authorize(Capability::Plugin(reference.original_name.clone()), format!("Invoke plugin tool {}", reference.original_name))?;
        ...
        let mut child = Command::new("node").arg(host).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().context("Node.js is required to run plugins")?;
        use std::io::Write;
        child.stdin.as_mut().context("plugin host stdin unavailable")?.write_all(request.as_bytes())?;
        let output = child.wait_with_output()?;
        let response: Value = serde_json::from_slice(&output.stdout).context("plugin host returned invalid JSON")?;
        if !response.get("ok").and_then(Value::as_bool).unwrap_or(false) { anyhow::bail!("plugin failed: {}", response.get("error").and_then(Value::as_str).unwrap_or("unknown error")); }
        Ok(response.get("result").cloned().unwrap_or(Value::Null).to_string())
    }

    fn plugin_host(&self) -> Result<PathBuf> {
        if let Ok(path) = env::var("TC_PLUGIN_HOST") { return Ok(PathBuf::from(path)); }
        ...
        if !path.exists() {
            ...
            fs::write(&path, include_str!("../../../packages/plugin-host/index.mjs"))?;
        }
        Ok(path)
    }
}

fn required_string(value: &Value, field: &str) -> Result<String> { value.get(field).and_then(Value::as_str).map(ToOwned::to_owned).with_context(|| format!("tool argument {field} must be a string")) }

#[cfg(test)]
mod tests {
    use super::*;
    struct Deny;
    impl Approval for Deny { fn approve(&self, _: &ApprovalRequest) -> Result<bool> { Ok(false) } }
    #[test]
    fn refuses_paths_outside_the_workspace() {
        let temp = tempfile::tempdir().unwrap();
        let executor = ToolExecutor::new(temp.path(), PermissionPolicy::with_read_access(), Arc::new(Deny)).unwrap();
