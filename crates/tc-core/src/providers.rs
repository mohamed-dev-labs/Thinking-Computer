use crate::{config::{ProviderKind, ResolvedProvider}, model::{ChatMessage, Role, ToolCall, ToolDefinition}};
...
pub async fn complete(provider: &ResolvedProvider, messages: &[ChatMessage], tools: &[ToolDefinition]) -> Result<ChatMessage> {
    match provider.kind {
...
fn remote_key(provider: &ResolvedProvider) -> Result<&str> {
    provider.api_key.as_deref().context(format!("{} requires an API key; use an environment variable or local configuration", provider.kind.as_str()))
}

fn message_text(message: &ChatMessage) -> Value {
    match message.role {
        Role::Tool => json!({"role":"tool","tool_call_id":message.tool_call_id,"content":message.content}),
...
async fn json_response(response: reqwest::Response) -> Result<Value> {
    ...
    if !status.is_success() { anyhow::bail!("provider request failed ({status}): {body}"); }
...
async fn openai(provider: &ResolvedProvider, messages: &[ChatMessage], tools: &[ToolDefinition]) -> Result<ChatMessage> {
    ...
    let tool_calls = message.get("tool_calls").and_then(Value::as_array).unwrap_or(&Vec::new()).iter().map(|call| ToolCall {
        ...
        arguments: serde_json::from_str(call.pointer("/function/arguments").and_then(Value::as_str).unwrap_or("{}")) .unwrap_or_else(|_| json!({})),
    }).collect();
...
fn anthropic_message(message: &ChatMessage) -> Option<Value> {
    match message.role {
        ...
        Role::Tool => Some(json!({"role":"user","content":[{"type":"tool_result","tool_use_id":message.tool_call_id,"content":message.content}]})),
    }
}

async fn anthropic(provider: &ResolvedProvider, messages: &[ChatMessage], tools: &[ToolDefinition]) -> Result<ChatMessage> {
    let endpoint = provider.base_url.clone().unwrap_or_else(|| "https://api.anthropic.com/v1/messages".into());
    let system = messages.iter().filter(|message| message.role == Role::System).map(|message| message.content.clone()).collect::<Vec<_>>().join("\n");
    let body = json!({
    ...
    let value = json_response(Client::new().post(endpoint).header("x-api-key", remote_key(provider)?).header("anthropic-version", ANTHROPIC_VERSION).json(&body).send().await?).await?;
    let blocks = value.get("content").and_then(Value::as_array).context("Anthropic response has no content")?;
    let content = blocks.iter().filter(|block| block.get("type").and_then(Value::as_str) == Some("text")).filter_map(|block| block.get("text").and_then(Value::as_str)).collect::<Vec<_>>().join("\n");
    let tool_calls = blocks.iter().filter(|block| block.get("type").and_then(Value::as_str) == Some("tool_use")).map(|block| ToolCall {
        id: block.get("id").and_then(Value::as_str).unwrap_or_default().to_string(), name: block.get("name").and_then(Value::as_str).unwrap_or_default().to_string(), arguments: block.get("input").cloned().unwrap_or_else(|| json!({})),
    }).collect();
    Ok(ChatMessage { role: Role::Assistant, content, tool_calls, tool_call_id: None, tool_name: None })
}
...
async fn gemini(provider: &ResolvedProvider, messages: &[ChatMessage], tools: &[ToolDefinition]) -> Result<ChatMessage> {
    let mut endpoint = Url::parse(&provider.base_url.clone().unwrap_or_else(|| format!("https://generativelanguage.googleapis.com/v1beta/models/{}:generateContent", provider.model)))?;
    endpoint.query_pairs_mut().append_pair("key", remote_key(provider)?);
    let instruction = messages.iter().filter(|message| message.role == Role::System).map(|message| message.content.clone()).collect::<Vec<_>>().join("\n");
    let body = json!({
    ...
    let parts = value.pointer("/candidates/0/content/parts").and_then(Value::as_array).context("Gemini response has no content parts")?;
    let content = parts.iter().filter_map(|part| part.get("text").and_then(Value::as_str)).collect::<Vec<_>>().join("\n");
    let tool_calls = parts.iter().filter_map(|part| part.get("functionCall")).map(|call| ToolCall {
        id: Uuid::new_v4().to_string(), name: call.get("name").and_then(Value::as_str).unwrap_or_default().to_string(), arguments: call.get("args").cloned().unwrap_or_else(|| json!({})),
    }).collect();
    Ok(ChatMessage { role: Role::Assistant, content, tool_calls, tool_call_id: None, tool_name: None })
}

async fn ollama(provider: &ResolvedProvider, messages: &[ChatMessage], tools: &[ToolDefinition]) -> Result<ChatMessage> {
    let endpoint = format!("{}/api/chat", provider.base_url.as_deref().unwrap_or("http://127.0.0.1:11434").trim_end_matches('/'));
    ...
    let value = json_response(Client::new().post(endpoint).json(&body).send().await?).await?;
    let message = value.get("message").context("Ollama response has no message")?;
    let tool_calls = message.get("tool_calls").and_then(Value::as_array).unwrap_or(&Vec::new()).iter().map(|call| ToolCall {
        id: Uuid::new_v4().to_string(), name: call.pointer("/function/name").and_then(Value::as_str).unwrap_or_default().to_string(), arguments: call.pointer("/function/arguments").cloned().unwrap_or_else(|| json!({})),
    }).collect();
    Ok(ChatMessage { role: Role::Assistant, content: message.get("content").and_then(Value::as_str).unwrap_or_default().to_string(), tool_calls, tool_call_id: None, tool_name: None })
}
