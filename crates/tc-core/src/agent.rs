use crate::{config::ResolvedProvider, memory::SessionStore, model::{ChatMessage, Role}, providers, tools::ToolExecutor};
use anyhow::Result;
...
impl Agent {
    pub fn new(provider: ResolvedProvider, tools: ToolExecutor, session: SessionStore, max_steps: usize) -> Self { Self { provider, tools, session, max_steps } }
    ...
    pub async fn run(&self, prompt: &str) -> Result<String> {
        ...
        for _ in 0..self.max_steps {
            ...
            if response.tool_calls.is_empty() { return Ok(response.content); }
            for call in &response.tool_calls {
                let result = match self.tools.execute(call).await { Ok(value) => value, Err(error) => format!("Tool error: {error}") };
