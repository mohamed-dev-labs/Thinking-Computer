...
impl SessionStore {
    pub fn local(session_id: Option<String>) -> Result<Self> {
        Self::in_directory(state_root(), session_id)
    }
    pub fn in_directory(root: impl AsRef<Path>, session_id: Option<String>) -> Result<Self> {
        ...
        fs::create_dir_all(&directory).with_context(|| {
            format!(
                "failed to create local session directory {}",
                directory.display()
            )
        })?;
        Ok(Self {
    ...
    pub fn append(&self, message: &ChatMessage) -> Result<()> {
        ...
        let mut file = OpenOptions::new()
            ...
            .append(true)
            .open(&self.path)
            .with_context(|| format!("failed to open local session {}", self.path.display()))?;
...
impl VmCapabilityProfile {
    pub fn collect() -> Self {
        ...
        let disks = Disks::new_with_refreshed_list()
            ...
            .collect();
        let available_tools = [
            "cargo", "rustc", "python3", "node", "npm", "git", "ollama", "docker", "podman",
            "ffmpeg",
        ]
        .into_iter()
        ...
        Self {
            captured_at: Utc::now(),
            platform: tc_system_bridge::platform_name(),
            cpu_architecture: tc_system_bridge::cpu_architecture(),
            kernel_version: System::kernel_version(),
            os_version: System::long_os_version(),
            cpu_count: tc_system_bridge::logical_cpu_count() as usize,
...
impl AgentMemory {
    ...
    pub fn in_directory(root: impl AsRef<Path>) -> Result<Self> {
        ...
        fs::create_dir_all(&root)
            .with_context(|| format!("failed to create agent memory at {}", root.display()))?;
        Ok(Self { root })
    }
    pub fn save_capability_profile(&self, profile: &VmCapabilityProfile) -> Result<()> {
        ...
        self.append_audit(
            "capability_profile",
            "Captured VM OS, resource, disk, process, and installed-tool indicators",
            true,
        )
    }
    ...
    pub fn remember(
        ...
        if topic.trim().is_empty() || content.trim().is_empty() {
            anyhow::bail!("memory topic and content must not be empty");
        }
        if looks_like_secret(content) {
            anyhow::bail!("refusing to persist content that appears to contain a secret");
        }
        ...
        let mut file = OpenOptions::new()
            .create(true)
            .append(true)
            .open(self.root.join("knowledge.jsonl"))?;
        writeln!(file, "{}", serde_json::to_string(&record)?)?;
        self.append_audit(
            "remember",
            &format!("topic={topic}; bytes={}", content.len()),
            true,
        )?;
        Ok(record)
    }
    pub fn recall(&self, limit: usize) -> Result<Vec<KnowledgeRecord>> {
        ...
        let mut records: Vec<KnowledgeRecord> = content
            .lines()
            .filter(|line| !line.trim().is_empty())
            .map(serde_json::from_str)
            .collect::<std::result::Result<_, _>>()?;
...
mod tests {
    ...
    fn writes_and_reads_a_local_jsonl_session() {
        ...
        store
            .append(&ChatMessage::text(Role::User, "hello"))
            .unwrap();
